...
# Iterates over all the documents in a corpus, transforming them by applying `__flatten_document()`
@typechecked
def flatten_corpus(path_in: pathlib.Path, path_out: pathlib.Path) -> None:
    ...
    with pb.ProgressBar(widgets = widgets) as bar:
        for file_name in path_in.iterdir():
            if u.is_corpus_document(file_name):
                bar.update(i)
                i = i + 1
                sentences = __flatten_document(file_name)
                u.write_document(path_out, file_name, sentences)

# Transforms a single document by removing all the newlines creating a document with a single line
@typechecked
...
if __name__ == '__main__':
    ...
    flatten_corpus(pathlib.Path(args.folder_in), pathlib.Path(args.folder_out))
