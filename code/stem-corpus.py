...
def stem_corpus(path_in: pathlib.Path, path_out: pathlib.Path) -> None:
    ...
    i = 1
    widgets = [ 'Stemming Corpus # ', pb.Counter(), ' ', pb.Timer(), ' ', pb.BouncingBar(marker = '.', left = '[', right = ']')]
    with pb.ProgressBar(widgets = widgets) as bar:
        for file_name in path_in.iterdir():
            if u.is_corpus_document(file_name):
                bar.update(i)
                i = i + 1
                sentences = __stem_document(file_name)
                u.write_document(path_out, file_name, sentences)

# Transforms a single document by stemming all the words
@typechecked
...
if __name__ == '__main__':
    ...
    stem_corpus(pathlib.Path(args.folder_in), pathlib.Path(args.folder_out))
